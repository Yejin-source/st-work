...
<body>
	<form method="post" action="${pageContext.request.contextPath}/joinMember">
		<table border="1">
            <!-- 아이디 -->
            <tr>
                <td>아이디</td>
                <td><input type="text" name="id" id="id"></td> <!-- 4자 이상 -->
            </tr>
            <!-- 비밀번호 -->
            <tr>
                <td>
	...
	<script>
		function join() {
			if(document.querySelector('#id').value.length < 4) {
				...
				alert('나이는 숫자를 입력하세요.');	
			} else if(document.querySelectorAll('.gender:checked').length == 0) { // .gender 중에 checked 된 게 없음
					alert('성별을 선택하세요.');
					...
					alert('이메일을 확인하세요.');
			} else if(document.querySelectorAll('#memo').length < 1) { 
				alert('메모를 입력하세요.');
			} else {
