...
@RestController // View가 없는 컨트롤러 -> Model값을 Json 문자열 값으로 반환
public class TestController {
	...
	public List<Emp> empList() {
		return empRepository.findAll(); // Emp Entity select / Emp 테이블 select (X)
	}
	
	@GetMapping("/deptList")
	public List<Dept> deptList() {
		return deptRepository.findAll(); // Emp Entity select / Emp 테이블 select (X)
	}
}
